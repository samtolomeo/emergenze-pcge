		...
		?>
			 ...
			 <div class="form-group  ">
				  <label for="cf">Seleziona dipendente comunale:</label> <font color="red">*</font>
								...
								<option name="cf" value="">Seleziona personale</option>
				<?php    
				while($r2 = pg_fetch_assoc($result2)) { 
				?>
			    <option name="cf" value="<?php echo $r2['matricola'];?>" ><?php echo $r2['cognome'].' '.$r2['nome'].' ('.$r2['settore'].' - '.$r2['ufficio'].' )';?></option>
				 <?php } ?>
				 </select>            
...
<?php
      ...
      <div class="modal-footer">
        ...
        <form autocomplete="off" action="report/nuovo_oNV.php" method="POST">
            ...
            <div class="form-group  ">
				  <label for="cf">Seleziona dipendente comunale 2:</label> <font color="red">*</font>
								<select name="cf2" id="cf2" class="selectpicker show-tick form-control" data-live-search="true" required="">
								<option  id="cf2" name="cf2" value="">Seleziona personale</option>
 ...
 http.onreadystatechange = function() {
  if(http.readyState === 4 && http.status === 200) {
      ...
      for(var i=0; i<json.length;i++){// ciclo che riempie di nuovo la select con id "comune" con le nuove option
      	...
      	//$('#cf2').append('<option value="' + json[i].matricola + '">' + json[i].cognome + ' ' + json[i].nome + '(' + json[i].settore + ' - ' + json[i].ufficio +')</option>'); //nome è definito nella stringa JSON
      }
      }
  }
 };
