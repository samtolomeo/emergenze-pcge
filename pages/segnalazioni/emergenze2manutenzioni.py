...
def get_response_from_provider(token, IdSegnalante, Descrizione, IdManufatto, CodViaDa, CivicoDa,  ColoreDa, LetteraDa):
    url = t.url
    #print(url)
   ...
   <soap:Body>
      <goad:InserimentoSegnalazione>
         ...
         <goad:manutenzioneSegnalazioneInput>
            ...
            <goad:IdSegnalante>{}</goad:IdSegnalante>
            <goad:IdManufatto>{}</goad:IdManufatto>
            <!--Optional:-->
            <goad:Descrizione>{}</goad:Descrizione>
            <goad:IdTipologiaIntervento>21</goad:IdTipologiaIntervento>
            <!--Optional:-->
            <goad:Matricola>emergenze</goad:Matricola>
            <goad:CodViaDa>{}</goad:CodViaDa>
            <!--Optional:-->
            <goad:CivicoDa>{}</goad:CivicoDa>
            <!--Optional:-->
            <goad:ColoreDa>{}</goad:ColoreDa>
            <!--Optional:-->
    ...
    """.format(IdSegnalante,IdManufatto,Descrizione,CodViaDa,CivicoDa,ColoreDa, LetteraDa)
...
def main():
    ...
    descrizione=''
    try:
        opts, args = getopt.getopt(sys.argv[1:], "mv:n:c:l:d:", ["help", "via=", "ncivico=", "colore=","lettera=", "descrizione="])
    except getopt.GetoptError:
        manual= 'emergenze2manutenzioni.py -v <codvia> -n <ncivico> -c <colore> -l <lettera> -d <descrizione>'
        print(manual)
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-m':
    ...
    response=get_response_from_provider(token,IdSegnalante, descrizione, id_manufatto, codvia, ncivico, colore, lettera)[0]
    id_segnalazione=get_response_from_provider(token,IdSegnalante, descrizione, id_manufatto, codvia, ncivico, colore, lettera)[1]
