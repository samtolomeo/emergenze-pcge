...
$query= "SELECT count(id) FROM users.v_squadre WHERE id_stato=1;";
$result = pg_query($conn, $query);
while($r = pg_fetch_assoc($result)) {
	$squadre_in_azione = $r['count'];	
}

$query= "SELECT count(id) FROM users.v_squadre WHERE id_stato=2;";
$result = pg_query($conn, $query);
while($r = pg_fetch_assoc($result)) {
	$squadre_disposizione = $r['count'];	
}

$query= "SELECT count(id) FROM users.v_squadre WHERE id_stato=3;";
$result = pg_query($conn, $query);
while($r = pg_fetch_assoc($result)) {
